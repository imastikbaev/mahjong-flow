  ...
  return (
    ...
    ">
      ...
      <div className="px-5 pt-5 pb-4 flex items-center justify-between border-b border-white/[0.06]">
        ...
        {/* Scope toggle */}
        <div className="flex rounded-md border border-white/[0.08] overflow-hidden text-[11px]">
          <ScopeButton
            label="Global"
            active={scope === 'global'}
      ...
      <div className="px-2 py-2 min-h-[200px]">
        ...
        {!loading && error && (
          <div className="flex flex-col items-center gap-3 py-8 text-center">
            ...
            <p className="text-xs font-light text-neutral-600">{error}</p>
            <button
              onClick={refetch}
              className="text-xs font-normal text-neutral-400 hover:text-neutral-200 transition-colors"
            >
              Retry
            </button>
          </div>
        ...
        {!loading && !error && rows.length === 0 && (
          <div className="flex flex-col items-center gap-2 py-10 text-center">
            ...
            <p className="text-xs font-light text-neutral-600 tracking-tight">
              {scope === 'local'
...
function LeaderboardRow({
  ...
  const rankColor =
    rank <= 3 ? 'text-neutral-300' : 'text-neutral-600';
  ...
  return (
    ...
    ">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-normal text-neutral-300 tracking-tight truncate">
          {row.users.nickname}
        </p>
        {row.users.city && (
          <p className="text-[10px] font-light text-neutral-600 truncate">
            {row.users.city}
          </p>
        )}
      </div>
      ...
      <div className="text-right shrink-0">
        <p className="text-sm font-normal text-neutral-300 tabular-nums tracking-tight">
          {row.score.toLocaleString()}
        </p>
        <p className="text-[10px] font-light text-neutral-600 tabular-nums">
          {mm}:{ss}
        </p>
      </div>
    </li>
    ...
    <div className="space-y-0.5 px-1 py-2 animate-pulse">
      {Array.from({ length: 5 }).map((_, i) => (
        <div key={i} className="flex items-center gap-3 px-3 py-2">
          <div className="w-5 h-3 rounded bg-white/[0.05]" />
          <div className="flex-1 space-y-1.5">
            <div className="h-3 rounded bg-white/[0.05] w-2/3" />
            <div className="h-2 rounded bg-white/[0.03] w-1/3" />
          </div>
          <div className="space-y-1.5 text-right">
            <div className="h-3 rounded bg-white/[0.05] w-12" />
            <div className="h-2 rounded bg-white/[0.03] w-10" />
          </div>
        </div>
      ))}
    </div>
